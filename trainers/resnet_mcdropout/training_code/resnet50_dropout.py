...
def apply_dropout(inputs, dropout_rate, filterwise_dropout):
  ...
  return tf.keras.layers.Dropout(    #***Training = True here***, this is what McClure wants me to change...
      dropout_rate, noise_shape=noise_shape)(
          inputs, training=True) #still sample at test time, everytime you make a prediction, ur using a diff model
...
def resnet50_dropout(input_shape,
  ...
  x = tf.keras.layers.Dense(   #Do i take this out???
      num_classes,
      activation=None,
