...
def BNN_predict(num_classes, predict_probs, true_labels, model_type):
    ...
    if model_type == 'multi_class':
        entropy = -np.sum(preds * np.log(preds + 1E-14), axis=1) #this is total entropy
    else:
    ...
    if model_type == 'multi_class': #aleatoric is the mean of the entropies
        aleatoric_depeweg = np.mean(-np.sum(predict_probs * np.log(predict_probs + 1E-14), axis=2), axis=0) #mean of entropies
    else:
