#The purpose of this file is to display the most uncertain images. 
...
pickle_save_path_mcdrop_imagenet = '/data/kraken/coastal_project/coastal_proj_code/orange_cable/imagenet_mcdrop_cable_1_20_WithImage.pickle'
...
def get_metrics_from_pickle(pickle_save_path, imgnet_flag): 
    ...
    for index, iteration_results in enumerate(results_list, start=1):
        ...
        #Least Uncertain 
        # corresponding_true_labels = true_labels_sort[9:]
        # corresponding_predicted_labels = pred_labels_sort[9:]
        # images_to_analyze = images_sort[9:]
    ...
    error = std 
    #plot the most uncertain...
    fig, axes = plt.subplots(2, 5, figsize=(8,10))
    # axes = axes.flatten()
    for i in range(2):  # Rows
        for j in range(5):  # Columns
            # axes[i, j].imshow(images_to_analyze[i * 5 + j]) keep?
    # image1 = cv2.imread('/data/kraken/coastal_project/coastal_proj_code/scatter_plots/mbt_camouflage.jpeg')
    # plt.imshow(image1[...,::-1])
            # axes[i, j].set_ticks([])  # Turn off axis labels for clarity
            # axes[i, j].set_xlabel(f'Image {i * 5 + j + 1}')   
            resized_image = cv2.resize(images_to_analyze[i * 5 + j], (150,150))
            axes[i, j].imshow(resized_image)
            
        
            axes[i, j].set_xlabel(f'True: {IMAGENET_CLASS_INT_TO_STR[corresponding_true_labels[i * 5 + j]]}, \n Predicted: {IMAGENET_CLASS_INT_TO_STR[corresponding_predicted_labels[i * 5 + j]]}',rotation=45)
    ...
    plt.suptitle("10 most Uncertain Pistachio Images (total entropy)", fontsize=16)
    # plt.tight_layout()
    plt.savefig(f"/data/kraken/coastal_project/coastal_proj_code/orange_cable/most_uncertain_images.png")
...
index_mcdrop_imagenet, mean_mcdrop_imagenet, error_mcdrop_imagenet = get_metrics_from_pickle(pickle_save_path_mcdrop_imagenet, imgnet_flag=False)
